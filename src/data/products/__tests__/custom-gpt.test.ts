...
describe('Custom Blog GPT Products', () => {
	...
	it('should have the Self-Serve Template priced at $199', () => {
		const template = customGptProducts.find(p => p.id === 'custom-blog-gpt-template');
	...
	it('should have the Standard Build service priced at $4,500', () => {
		const standard = customGptProducts.find(p => p.id === 'custom-blog-gpt-build-standard');
	...
	it('should have the Engine Build service priced at $12,000', () => {
		const engine = customGptProducts.find(p => p.id === 'custom-blog-gpt-build-engine');
