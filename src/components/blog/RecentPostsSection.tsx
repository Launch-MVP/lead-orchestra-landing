	...
	return (
		...
		>
			<h3
				...
				className="mb-5 font-semibold text-gray-900 text-lg dark:text-white"
			>
				{title}
			</h3>
			<div className="flex flex-col gap-4">
				{recentPosts.map((post) => (
					<Link
						...
						className="group flex items-start gap-3 rounded-xl border border-slate-200/50 bg-slate-100/50 p-3 transition hover:border-primary/60 hover:bg-primary/10 dark:border-white/5 dark:bg-white/5"
						target="_blank"
					...
					>
						...
						<div className="flex flex-col">
							<h4 className="font-semibold text-gray-900 text-sm transition-colors group-hover:text-primary dark:text-white">
								{post.title}
							</h4>
							<p className="mt-0.5 text-muted-foreground text-xs">
								{(() => {
									...
									if (typeof raw === "number")
										return new Date(raw * 1000).toLocaleDateString();
