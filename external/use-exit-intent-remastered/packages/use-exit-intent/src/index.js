...
export * from './types';
import { contexts, createDebounce, createIdleEvents, defaultSettings, isDesktop, isMobile, processHandlersByDeviceContext, removeIdleEvents, secondsToMiliseconds, } from './utils';
const truthyValues = new Set(['1', 'true', 'on', 'yes']);
const debugEnabled = () => truthyValues.has((process.env.NEXT_PUBLIC_EXIT_INTENT_DEBUG ?? 'false').trim().toLowerCase());
...
export function useExitIntent(props) {
    const { cookie: cookieOverrides, desktop: desktopOverrides, mobile: mobileOverrides, } = props ?? {};
    ...
    const mobileDelaySeconds = settingsMobile?.delayInSecondsToTrigger ?? defaultSettings.mobile.delayInSecondsToTrigger;
    const mobileTriggerOnIdle = settingsMobile?.triggerOnIdle ?? defaultSettings.mobile.triggerOnIdle;
    const desktopDelaySeconds = settingsDesktop?.delayInSecondsToTrigger ?? defaultSettings.desktop.delayInSecondsToTrigger;
    const desktopMouseLeaveDelaySeconds = settingsDesktop?.mouseLeaveDelayInSeconds ?? defaultSettings.desktop.mouseLeaveDelayInSeconds;
    const desktopTriggerOnIdle = settingsDesktop?.triggerOnIdle ?? defaultSettings.desktop.triggerOnIdle;
    const desktopTriggerOnMouseLeave = settingsDesktop?.triggerOnMouseLeave ?? defaultSettings.desktop.triggerOnMouseLeave;
    const desktopUseBeforeUnload = settingsDesktop?.useBeforeUnload ?? defaultSettings.desktop.useBeforeUnload;
    const handleExitIntent = useCallback(() => {
        ...
        for (const handler of handlersRef.current) {
            ...
            const isDefault = contextsForHandler.filter((context) => context !== contexts.onDesktop && context !== contexts.onMobile).length === 0;
    ...
    const registerHandler = useCallback((handler) => {
        ...
        const handlerAlreadyPushed = handlers.find((registeredHandler) => registeredHandler.id === handler.id);
    ...
    useEffect(() => {
        ...
        const cleanups = [];
        if (isMobile()) {
            const { execute, abort } = createDebounce(handleExitIntent, secondsToMiliseconds(mobileDelaySeconds));
            if (shouldNotTrigger.current) {
        ...
        if (isDesktop()) {
            const { execute, abort } = createDebounce(handleExitIntent, secondsToMiliseconds(desktopDelaySeconds));
            let mouseLeaveTimer;
